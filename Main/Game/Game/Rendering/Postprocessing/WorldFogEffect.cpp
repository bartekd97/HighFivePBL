...
void WorldFogEffect::LateUpdateNoise(Event& event)
{
...
void WorldFogEffect::Init()
{
	...
	shader = ShaderManager::GetShader("PPWorldFog");
	shader->use();
	shader->setInt("depthMap", 0);
	shader->setInt("noiseMap", 1);
	//shader->setInt("gPosition", 2);

	EventManager::AddListener(METHOD_LISTENER(Events::General::POST_UPDATE, WorldFogEffect::LateUpdateNoise));
}

bool WorldFogEffect::Process(
